...
train.filename <- paste("./output/adult_train_", task.attr, tag.bias, "_norm.txt", sep="")
...
# after processing
result.file <- paste("./output/ans_adult_", task.attr, tag.bias, "_norm_", epsilon, ".txt", sep = "")
result <- read_objective_result(result.file)
test.filename <- paste("./output/adult_test_", task.attr, tag.bias, "_norm.libsvm", sep="")
...
yyy<-compute_svm_with_bias(result$w, data.test[, c(2:ncol(data.test))])
miss.rate <- compute_miss_rate(yyy, data.test[,1])
