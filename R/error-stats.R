ErrorStats <- setRefClass(
  ...
  methods = list(
    ...
    add_svm_record = function(attr.content, attr.name, miss.rate, timestamp = NULL) {
      ...
      .self$svm.miss.rates <- rbind(.self$svm.miss.rates
                                    , list(attr.content, attr.name
                                           , miss.rate, timestamp))
    ...
    write_out_avg_svm_miss_rates = function(jtree.file) {
      ...
      write_out_svm_avg_miss_rates <- function(attr.content
                                               , attr.name
                                               , avg.miss.rate){
    ...
    add_error_stats_record_from_file = function(err.stats){
      .self$errors <- rbind(.self$errors, 
                            list(as.character(err.stats$query.type)
    ...
    add_error_stats_record = function(qtype, jtree, prob.dist, k) {
      ...
      L2.error.consistent <- sum(prob.dist$L2.error.consistent) / 
        length(prob.dist$L2.error.consistent)
      total.var <- sum(prob.dist$var)/length(prob.dist$var)
      total.var.consistent <- sum(prob.dist$var.consistent) / 
        length(prob.dist$var.consistent)
    ...
    write_out_avg_errors = function(qtype, jtree, k){
      ...
      eps2.tag <- paste(unlist(strsplit(as.character(.self$epsilon.2)
                                        , split = "\\."))
                      , sep = "", collapse = "")
      out.file <- paste(.self$out.dir, jtree, "-eps2-", .self$epsilon.2 
